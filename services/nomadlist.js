...
// This function will be called when someone clicks on a saved city.
// It will fetch information about co-working facilities in that city.
function getWorkPlaces(req, res, next) {
  console.log('getting work places from API')
  fetch(`https://nomadlist.com/api/v2/list/cities/${req.params.slug}/places/work`)
  .then(r => r.json())
  .then((workData) => {
    res.workData = workData.result;
    next();
  })
  .catch(err => console.log(err));
}
